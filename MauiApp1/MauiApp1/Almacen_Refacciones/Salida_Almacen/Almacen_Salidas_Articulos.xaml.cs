...
namespace iAlmacen.Almacen_Refacciones.Salida_Almacen
{
	public partial class Almacen_Salidas_Articulos : ContentPage
	{
		...
		public Almacen_Salidas_Articulos(string Nivel3, string Nivel4, ItemsViewModel_ArticuloSalida vm_ArticuloSalida)
		{
			...
			BindingContext = viewModel_ArticuloSalida = vm_ArticuloSalida;
		}
		...
		private async Task obtenerDatosArticulo(string codArticulo)
		{
			...
			try
			{
				HttpResponseMessage response = ConfigAPI.GetAPI("GET", "api/Operacion/GET", Parametros, "wsp_DatosArticulo").Result;
		...
		private async Task obtenerDatosControl(string codControl)
		{
			...
			else
			{
				...
				HttpResponseMessage response = ConfigAPI.GetAPI("GET", "api/Operacion/SQL", Parametros, "wsp_execute_qwerty", "detalle_documentos_almacen", Condicion, "SELECT", "").Result;
				using (StreamReader reader = new StreamReader(response.Content.ReadAsStreamAsync().Result))
				{
					if (response.StatusCode == HttpStatusCode.NotFound) return;
					string resp = reader.ReadToEnd();
		...
		private async void OnTextResultUbiGenerated(string result)
		{
			MainThread.BeginInvokeOnMainThread(async () =>
			{
				...
				await obtenerDatosUbicacion(result);
			});
		}
		...
		private async void OnTextResultCtrlGenerated(string result)
		{
			MainThread.BeginInvokeOnMainThread(async () =>
			{
				...
				await obtenerDatosControl(result);
			});
		}
		...
		private async void btnAgregar_Clicked(Object sender, EventArgs e)
		{
			...
			else
			{
				...
				viewModel_ArticuloSalida.AddItem(item_ArticuloSalida);
		...
		private async void btnmanual_Clicked(Object sender, EventArgs e)
		{
			...
			if (result == null) { /* Cancelar */ }
			else if (string.IsNullOrWhiteSpace(result)) { await DisplayAlertAsync("Error", "Debe ingresar una clave valida.", "OK"); }
			else
			{
				Codigo = result;
				try
				{
					HttpResponseMessage response = ConfigAPI.GetAPI("GET", "api/Operacion/GET", Codigo, "wsp_DatosArticulo").Result;
					using (StreamReader reader = new StreamReader(response.Content.ReadAsStreamAsync().Result))
					{
						if (response.StatusCode == HttpStatusCode.NotFound) return;
						string resp = reader.ReadToEnd();
						...
						if (dt.Rows.Count == 0)
						{
							await DisplayAlertAsync("Informacion", $"La clave del Articulo {result} no existe, Favor de Validar", "OK");
						}
						foreach (DataRow r in dt.Rows)
						{
							if (r[16].ToString() == "1")
